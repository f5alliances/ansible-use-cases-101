...
Next, add the first ``task``. This task will use the ``bigip_node``
...
Login information for the BIG-IP: - username: admin - password: ansible

The list of nodes can be found by navigating the menu on the left. Click
on Local Traffic-> then click on Nodes.
